...
pygame.init()
pygame.display.set_caption('Продержался')
...
pygame.time.set_timer(BONFIREEVENT, 0)
TEMPEVENT = pygame.USEREVENT + 2
pygame.time.set_timer(TEMPEVENT, 0)
MOBEVENT = pygame.USEREVENT + 3
pygame.time.set_timer(MOBEVENT, 0)
...
def level(difficult):
    ...
    class AnimatedSprite(pygame.sprite.Sprite):
        ...
        def update(self, *args):
            ...
            self.x += self.vx // FPS
            self.y += self.vy // FPS
            if (args and args[0].type == pygame.MOUSEBUTTONDOWN and self.rect.collidepoint(args[0].pos) and player.axe
                    and abs(self.rect.x - 320 * pixsel) < 120 * pixsel and
                    abs(self.rect.y - 200 * pixsel) < 120 * pixsel):
                self.hp -= 1
                self.x -= self.vx
                self.y -= self.vy
            if abs(self.rect.x - 320 * pixsel) < 50 * pixsel and abs(self.rect.y - 200 * pixsel) < 50 * pixsel:
                player.hp -= 0.1
            if self.hp <= 0:
    ...
    class ChristmasTree(pygame.sprite.Sprite):  ##класс блока - елки
        ...
        def update(self, *args):
            ...
            if (args and args[0].type == pygame.MOUSEBUTTONDOWN and self.rect.collidepoint(args[0].pos)
                    and abs(self.x - player.x) < 300 and abs(
                        self.y - player.y - 500) < 230 and not self.chopped and player.axe):
    ...
    while running:
        ...
        screen.blit(Font.render(f' {int(player.hp)}       {int(player.temp)}',
                                1, (0, 0, 0)), (10, 10))
        time += 1
        pygame.display.set_caption(f'Продержался {time // FPS // 60}:{time // FPS % 60}')
        pygame.display.flip()
        if player.hp <= 0:
...
##сложность 0: общая обратная легкая 6,нормальная 5, трудная 4, сложная 3, невозможная 2
level([1])
level([1])
level([1])
pygame.quit()
